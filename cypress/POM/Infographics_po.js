class Infographics {
    ...
    project() {
        ...
        cy.get("[href] .is-between").click({force:true}).wait(5000);
      }

      settingsInfo() {
        cy.get('.pm-dropdown > .is-btn').click({force:true}).wait(2000);
        cy.get("li:nth-of-type(7) > a").click({force:true}).wait(4000); // Infographic section inside settings
        // cy.go(-1); //To Go Back from the page 
        return this;
      }

      addinfographics() {
        cy.get(".is-btn.is-btn_icon.is-btn_link > span").click({force:true}).wait(2000); // click on create button
        ...
        cy.get(".naxatw-grow p span").click({force:true}).wait(2000);
        cy.get('.select-list > :nth-child(3)').click({force:true});  // Selecting 'Ethnicity Of Household Head' option from the dropdown
        ...
        cy.get(".naxatw-toggle-label div").click({force:true}).wait(2000);
        cy.get(".is-btn_primary").click({force:true});
      ...
      editinfographics() {
        ...
        cy.get(".false.is-circle > .material-icons").click({force:true}).scrollIntoView().wait(2000) // click on three dots
        cy.get("tbody tr:nth-of-type(1) div .material-icons").eq(0).click({force:true}); //clicking the 3 dots 
        cy.get(".mt-1.pm-dropdown_menu > li:nth-of-type(1) > a").click({force:true}).wait(3000); //clicking the edit 
        cy.get(":nth-child(2) > .pm-group > .naxatw-space-y-2 > .pm-control").clear().type("Naxa_Infographics").wait(3000);
        cy.get(".common-button.is-btn.is-btn_primary").click({force:true});
        cy.scrollTo('top');
        cy.get('.Toastify__toast-container').should('be.visible') // Ensure the toast container is visible
      ...
      dashboard() {
        cy.get('.dbd-breadcrumb > :nth-child(2) > a').click({force:true}).wait(4000);
      ...
      infographiclist() {
        ...
        cy.get('.pm-dropdown > .is-btn').click({force:true}).wait(2000); // click on settings
        cy.get("li:nth-of-type(7) > a").click({force:true}).wait(2000); // Infographic section inside settings
        cy.get('.pm-control').type("Naxa_Infographics"); // type infogrpahic name in search 
        cy.get(".false.is-circle > .material-icons").click({force:true}).scrollIntoView().wait(2000) // click on three dots
        cy.get(".mt-1.pm-dropdown_menu > li:nth-of-type(1) > a").click({force:true}).wait(2000) //click on edit button
        cy.get('.naxatw-toggle').click({force:true}).wait(2000) // click on toggle button
        cy.get('.common-button').click({force:true}) // click on save button
        ...
        cy.get('.naxatw-gap-x-2 > [type="button"]').click({force:true}) //click on infogrpahic from the project dashboard
        cy.get('.pm-control').type("Naxa_Infographics").wait(5000).clear();
        cy.get('.pm-select_item').click({force:true}).wait(3000); //click on selet ward dropdown
        cy.get(".left-dropdown.pm-select_list > li:nth-of-type(1)").click({force:true}).wait(5000);
        return this;
      }
      ...
      deleteinfographic() {
        ...
        cy.get('.pm-control').type("Naxa_Infographics").wait(5000);
        return this;
      }
